"""
AgentGuard SDK
Just a few lines of code to wrap your agent and get eBPF-powered protection.
Works with LangChain, OpenAI, AutoGen, and more.
"""
...
class Policy(IntEnum):
    ...
    RESTRICTED  = 1   # HTTPS only, no exec, no external net, blocks on injection
...
class AgentGuard:
    ...
    Registers the calling process with the AgentGuard daemon so the
    eBPF kernel layer applies the correct policy to all network traffic
    and syscalls originating from this PID.

    Example:
        guard = AgentGuard()
        guard.register("my-agent", Policy.RESTRICTED)
        # ... run agent ...
        guard.unregister()  # or let atexit handle it
    """
    ...
    def register(
        ...
        Args:
            name:   Human-readable agent name (e.g. "my-langchain-agent")
            policy: Policy tier to apply (default: RESTRICTED)
            tags:   Optional metadata tags for dashboards

        Returns:
            True if registration succeeded, False if daemon unreachable.
            AgentGuard is designed to fail open — if the daemon is down,
            the agent runs unguarded rather than being blocked.
        """
...
def langchain_callback(guard: "AgentGuard"):
    ...
    Usage:
        ...
        agent_executor = AgentExecutor(
            agent=agent,
            tools=tools,
            callbacks=[langchain_callback(guard)],
        )
    """
    try:
