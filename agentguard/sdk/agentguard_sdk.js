 ...
 * ========================================
 * One-line integration for Node.js AI agent frameworks.
 *
 * Works with:
 *   - LangChainJS
 *   - Vercel AI SDK
 *   - OpenAI Node SDK
 *   - Anthropic Node SDK
 *   - Any custom Node.js agent
 *
 * Usage (CommonJS):
 *   const { AgentGuard, Policy } = require('./agentguard_sdk');
 *   const guard = new AgentGuard({ daemonUrl: 'http://localhost:8080' });
 *   await guard.register('my-node-agent', Policy.RESTRICTED);
 *
 * Usage (ESM / TypeScript):
 *   import { AgentGuard, Policy } from './agentguard_sdk.js';
 *   const guard = new AgentGuard();
 *   await guard.register('my-node-agent', Policy.TRUSTED);
 *
 * Environment variables:
 *   AGENTGUARD_URL        Daemon URL      (default: http://localhost:8080)
 *   AGENTGUARD_POLICY     Policy name     (default: restricted)
 *   AGENTGUARD_AGENT_NAME Agent name      (default: node-agent)
 *   HTTP_PROXY            Proxy URL       (set by AgentGuard.useProxy())
 *   HTTPS_PROXY           HTTPS proxy URL (set by AgentGuard.useProxy())
 *
 * Proxy setup for HTTP interception:
 *   guard.useProxy();   // sets HTTP_PROXY + HTTPS_PROXY env vars
 *   // Then use node-fetch, axios, or undici — they all respect these vars
 *
 * TypeScript types are inline — no separate .d.ts file needed.
 */
