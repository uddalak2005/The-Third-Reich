...
# Ensure proxy/ dir is on sys.path — prevents ag_proxy import errors
import sys as _sys
from pathlib import Path as _Path
_proxy_dir = str(_Path(__file__).parent)
if _proxy_dir not in _sys.path:
    _sys.path.insert(0, _proxy_dir)
        ...
        try:
            ...
            if first_chunk:
                ...
                if score > 0:
                    ...
                    log.info("https_request_clean", host=host, agent=detail.get("agent"))
            ...
            # Log successful completion of the relay if not already blocked
            event_store.push({
                "ts":                int(time.time_ns()),
                "event":             "proxy_request",
                "verdict":           "allow",
